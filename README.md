...
- ✅ **降级机制**：高阈值检测失败时自动降级到中等阈值，确保稳定性

### 🔧 **技术改进详情**
- **透明度检测**：从 `alpha > 10` 提升到 `alpha > 200` (高阈值) / `alpha > 128` (中等阈值)
- **主体识别算法**：新增 `getMainSubjectBounds()` 方法，替代原有的 `getNonTransparentBounds()`
- **边距优化**：从10%减少到5%，更紧凑的裁剪效果
- **像素级分析**：直接分析Alpha通道值，确保主体检测精度

详细信息请参考：[图像裁剪问题修复报告](docs/image-cropping-fix.md)
