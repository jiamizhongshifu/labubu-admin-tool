...
/// 识别会话
struct LabubuRecognitionSession: Codable, Identifiable {
    let id = UUID()
    let startTime: Date
    var endTime: Date?
    var recognitions: [LabubuRecognitionResult]
    var metrics: LabubuPerformanceMetrics
    
    init() {
        self.startTime = Date()
        self.recognitions = []
        self.metrics = LabubuPerformanceMetrics()
    }
    
    mutating func addRecognition(_ result: LabubuRecognitionResult) {
    ...
    private mutating func updateMetrics() {
        metrics = LabubuPerformanceMetrics()
        // 更新性能指标的逻辑
    }
...
enum LabubuRecognitionState {
    ...
    case completed(LabubuRecognitionResult)
    case failed(String)
    ...
    static func == (lhs: LabubuRecognitionState, rhs: LabubuRecognitionState) -> Bool {
        switch (lhs, rhs) {
        case (.idle, .idle),
             (.preprocessing, .preprocessing),
             (.quickDetection, .quickDetection),
        ...
        case (.completed(let lhsResult), .completed(let rhsResult)):
            return lhsResult.seriesId == rhsResult.seriesId
...
extension LabubuRecognitionResult {
    /// 是否为高置信度结果
